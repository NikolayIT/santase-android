...
/**
 * PlayCardExecutor abstract class. Provides the mechanism to be executed one by
 * one PlayCardMethod methods stored in an collection and to return the first
 * not null result value returned from the iteration. Also provides the facility
 * the user to write own code which is executed before the result to be
 * returned.
 * 
...
public abstract class PlayCardExecutor implements PlayCardMethod {
	...
	/**
	 * Handler method providing the user to write additional code which is
	 * executed after the getPlayerCard(Player).
	 * 
	 * @param player for which is called the executor.
	 * @param opposite player.
