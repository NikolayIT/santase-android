...
/**
 * BaseMethod class. Based class of all AI methods. Contains several helper
 * methods used in the concrete methods.
 * 
...
public abstract class BaseMethod implements PlayCardMethod {
	 ...
	 * @return boolean true if can play without loose (check opposite player) or
	 *         false.
	 */
	...
	protected boolean isBestSuitCardLeft(final Player player, Card card) {
		for (RankIterator iterator = Rank.iterator(); iterator.hasNext();) {
			...
			if (rank.compareTo(card.getRank()) > 0) {
				...
				if (!isTheLastCard &&  getRival(player).getHands().findCard(rank, card.getSuit()) == null && player.getHands().findCard(rank, card.getSuit()) == null) {
					return false;
				}
			}
		}
